	...
	Opa5.createPageObjects({
		onTheMasterPage: {
			...
			actions: {
				...
				iPressTheGroupButton: function() {
					return this.waitFor({
						...
						actions: new Press(),
						errorMessage: "Did not find 'Group' button."
				...
				iGroupAfter: function(groupKey) {
					return this.waitFor({
						searchOpenDialogs: true,
						controlType: "sap.ui.core.Item",
						matchers: function(oItem) {
							return (oItem.getProperty("text") === oItem.getModel("i18n").getProperty(groupKey));
						},
						actions: new Press(),

						success: function(oItem) {
							Opa5.assert.ok(true, "Grouped after '" + oItem[0].getProperty("text") + "'");
						},
						errorMessage: "Could not group the List"
					});
				},

				iFilterOn: function(filterKey) {
					return this.waitFor({
						searchOpenDialogs: true,
						controlType: "sap.ui.core.Item",
						matchers: function(oItem) {
							return (oItem.getProperty("text") === oItem.getModel("i18n").getProperty(filterKey));
						},
						actions: new Press(),

						success: function(oItem) {
							Opa5.assert.ok(true, "Filter on '" + oItem[0].getProperty("text") + "'");
						},
						errorMessage: "Could not filter the List"
					});
				},
						...
						success: function(aEntityData) {
							this.waitFor({
								...
								success: function(oList) {
									var sCurrentId,
										aItemsNotInTheList = aEntityData.filter(function(oObject) {
											return !oList.getItems().some(function(oListItem) {
												return oListItem.getBindingContext().getProperty("Guid") === oObject.Guid;
											});
										});

									if (!aItemsNotInTheList.length) {
										// Not enough items all of them are displayed so we take the last one
										sCurrentId = aEntityData[aEntityData.length - 1].Guid;
									} else {
										sCurrentId = aItemsNotInTheList[0].Guid;
									}
									...
									oCurrentItem.bindingPath = "/" + oList.getModel().createKey("MessageResultSet", {
										Guid: sCurrentId
									});
				...
				iSearchForTheFirstObject: function() {
					...
					return this.waitFor({
						...
						success: function(oList) {
							...
							return this.iSearchForValue(new EnterText({
				...
				iTypeSomethingInTheSearchThatCannotBeFoundAndTriggerRefresh: function() {
					...
					}), fireRefreshButtonPressedOnSearchField]);
				},
				...
				iRememberTheListItem: function(oListItem) {
					...
					this.getContext().currentItem = {
						...
						title: oBindingContext.getProperty("Description")
					};
				},
			...
			assertions: {
				...
				theFilterToolBarShouldBeVisible: function() {
					return this.waitFor({
						...
						errorMessage: "Filter Bar is not visible"
					});
				},
				...
				iShouldSeeTheGroupMenu: function() {
					...
					return this.waitFor({
						...
						matchers: function(oList) {
							...
							return (
								oList.getItems().length === 4 &&
								oList.getItems()[0].getText() === oList.getModel("i18n").getProperty("GROUP_NONE") &&
								oList.getItems()[1].getText() === oList.getModel("i18n").getProperty("GROUP_PRIORITY") &&
								oList.getItems()[2].getText() === oList.getModel("i18n").getProperty("GROUP_STATUS") &&
								oList.getItems()[3].getText() === oList.getModel("i18n").getProperty("GROUP_LAST_CHANGE_DATE"));

						},
						success: function(oList) {
							Opa5.assert.ok(true, "The Group Select List is visible");
						},
						errorMessage: "The Group Select List was not found"
					});

				},

				iShouldSeeTheFilterMenu: function() {
					...
					return this.waitFor({
						...
						matchers: function(oList) {
							...
							return (
								oList.getItems().length === 7 &&
								oList.getItems()[0].getText() === oList.getModel("i18n").getProperty("FILTER_ALL_OPEN") &&
								oList.getItems()[1].getText() === oList.getModel("i18n").getProperty("FILTER_NEW") &&
								oList.getItems()[2].getText() === oList.getModel("i18n").getProperty("FILTER_IN_PROCESS") &&
								oList.getItems()[3].getText() === oList.getModel("i18n").getProperty("FILTER_CUSTOMER_ACTION") &&
								oList.getItems()[4].getText() === oList.getModel("i18n").getProperty("FILTER_PROPOSED_SOLUTION") &&
								oList.getItems()[5].getText() === oList.getModel("i18n").getProperty("FILTER_WITHDRAWN") &&
								oList.getItems()[6].getText() === oList.getModel("i18n").getProperty("FILTER_CLOSED")

							);

						},
						success: function(oList) {
							Opa5.assert.ok(true, "The Filter Select List is visible");
						},
						errorMessage: "The Filter Select List was not found"
					});
				...
				theListShouldBeSortedAscendingOnName: function() {
					return this.theListShouldBeSortedAscendingOnField("Description");
				},

				theListShouldBeSortedOnField: function(sField, sortOrder) {
				...
				theGroupedListShouldBeSortedOnField: function(sField, sortOrder) {

					function getItemsWithoutHeader(oItems) {
					...
					function fnCheckSort(oList) {
						...
						var itemsOnly = getItemsWithoutHeader(oList.getItems());

						return itemsOnly.every(fnIsOrdered);
					}
				...
				theListShouldOnlyContainItemsInStatus: function(sStatus) {

					function fnCheckFilter(oList) {
						var fnHasCorrectStatus = function(oElement) {

							return oElement.getBindingContext().getProperty("Concatstatuser") === sStatus;
						};
						var aItems = oList.getItems();

						return aItems.every(fnHasCorrectStatus);
					}
					...
					return this.waitFor({
						...
						success: function() {
							Opa5.assert.ok(true, "Master list only contains items in status '" + sStatus + "'.");
						},
						errorMessage: "Master list does not only contail items with status value '" + sStatus + "'."
					});
				},
				...
				theListShouldHaveAllEntries: function() {
					...
					// retrieve all MessageResultSet to be able to check for the total amount
					...
					return this.waitFor({
						...
						success: function(oList) {
							...
							this.waitFor({
								...
								matchers: new PropertyStrictEquals({
									name: "title",
									value: "My Incidents (" + iExpectedLength + ")"
								}),
								success: function() {
